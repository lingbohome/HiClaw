...
def push_local(sync: FileSync, since: float = 0) -> list[str]:
    ...
    for path in local_dir.rglob("*"):
        ...
        # ── SQLite .db: safe backup before push ──────────────────────
        # Live databases cannot be copied safely while the agent writes
        # to them.  Use sqlite3's online backup API to create a consistent
        # snapshot, then push the snapshot.
        if rel.suffix == ".db":
            import sqlite3
            import tempfile
            db_src = str(path)
            ...
            try:
                ...
                snapshot = Path(tmp_name).read_bytes()
                if remote == snapshot.decode(errors="replace"):
                    continue
                dest = sync._object_path(key)
                _mc("cp", tmp_name, dest, check=True)
                pushed.append(str(rel))
                logger.debug("Pushed %s -> %s (safe backup)", rel, dest)
            except Exception as exc:
                logger.debug("push_local: safe-backup failed for %s: %s", rel, exc)
            finally:
                try:
                    os.unlink(tmp_name)
                except OSError:
                ...
                try:
                    if worker_name:
                        ...
                        if status not in ('in_progress', 'submitted'):
                            continue  # sync running and submitted tasks (signals + review files)
                except Exception:
