...
- **Git operations go through git-delegation — you have NO git credentials** — never run `git clone`, `git push`, `git pull`, `git fetch`, or create PRs directly. You will get 403/404/permission errors because your container lacks GitHub tokens and SSH keys. Instead, use the `git-delegation` skill: send a `git-request:` message to the coordinator with the workspace path and operations needed. The Manager will execute them on your behalf. Example: "task-{id} git-request: workspace: ~/shared/tasks/{id}/workspace/{repo} operations: - git push origin feature-branch".
...
- **When your coordinator @mentions you with a question** (e.g. status check, heartbeat), you MUST @mention them in your reply. This is NOT "mid-task progress" — they explicitly asked and cannot see your response without an @mention. After replying, continue working through your plan.md autonomously.
