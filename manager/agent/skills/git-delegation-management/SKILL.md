...
- **Fallback**: If `HICLAW_GITHUB_TOKEN` is set and no credential helper is detected, the Manager startup script configures `url."https://TOKEN@github.com/".insteadOf "https://github.com/"`. This transparently rewrites all HTTPS GitHub URLs to include the token, ensuring git-delegation works even on clean hosts without pre-configured git auth.
