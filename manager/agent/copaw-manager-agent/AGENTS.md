...
- **History context: read it, but only @mention based on the Current message sender** — history tells you what happened while you were away. Before responding to the current message (heartbeat, admin DM, etc.), scan the history for unanswered Worker questions or blockers. If a Worker asked for something (repo URL, credentials, spec clarification) and you never answered: address it FIRST, then handle the current message. Ignoring a Worker's question because "it's in the history section" is how tasks stall.
...
- **HICLAW_GIT_PROXY — use it, never remove it** — this env var contains a proxy address (e.g. `socks5://proxy:7890`) configured by the admin for bypassing network restrictions (GFW, corporate firewall). It is NOT just for git — use it for ANY network operation that fails due to unreachable hosts: `curl --proxy`, `wget -e use_proxy=yes`, `pip install --proxy`, `npm --proxy`, web research, API calls to blocked domains, etc. If a Worker asks for the proxy address to resolve network issues, provide `echo $HICLAW_GIT_PROXY`. NEVER remove this proxy, unset it, or modify git proxy config — network failures are transient, retry first, report to admin if persistent.
...
- **NEVER assign tasks to Workers by writing @worker mentions in admin DM reply text** — Workers cannot see DM messages. Always use `copaw channels send` with the Worker's `roomID` from `hiclaw get workers -o json`. In admin DM sessions, your final reply should only confirm to admin; actual task dispatch goes to the Worker Room via CLI.
